  ...
  const CustomTooltip: FC<any> = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      if (data.source && data.target) {
        // Link tooltip
        return (
          <div className="bg-background/80 backdrop-blur-sm p-2 border border-border rounded shadow-lg text-sm">
            <p className="font-semibold">{data.label}</p>
            {data.riskScore !== undefined && <p>Risk Score: {Math.round(data.riskScore)}</p>}
          </div>
        )
      }
      if (data.name) {
  ...
  return (
    <Card className="mt-3">
      <CardHeader className="pb-2">
        <CardTitle>{title || "Funds Flow Analysis"}</CardTitle>
      </CardHeader>
      <CardContent>
        {sankeyData.links.length > 0 ? (
          ...
          <div className="flex items-center justify-center h-64">
            <p className="text-muted-foreground">No transaction data available for flow analysis.</p>
          </div>
        )}
      </CardContent>
    </Card>
