  ...
  if (!hasEntityData) {
    return (
      <Card className={`${theme === 'light' ? 'bg-white border-gray-200' : 'bg-gray-800 border-gray-700'} rounded-2xl`}>
        <Empty
          image={<UserOutlined style={{ fontSize: 40, color: theme === 'light' ? '#6b7280' : '#6b7280' }} />}
          description={
  ...
  return (
    <Card className={`${theme === 'light' ? 'bg-white border-gray-200' : 'bg-gray-800 border-gray-700'} rounded-2xl`}>
      <div className="flex items-center mb-4">
        ...
        <div>
          <div className={`${theme === 'light' ? 'text-gray-900' : 'text-white'} font-semibold text-lg`}>{name}</div>
          <div className={`${theme === 'light' ? 'text-gray-500' : 'text-gray-500'} text-sm`}>{type}</div>
        </div>
      </div>
      {description && (
        <div className={`${theme === 'light' ? 'text-gray-500' : 'text-gray-500'} mb-2`}>{description}</div>
      )}
      {website && (
        <div className={`${theme === 'light' ? 'text-gray-500' : 'text-gray-500'} mb-1`}>
      ...
      {phone && (
        <div className={`${theme === 'light' ? 'text-gray-500' : 'text-gray-500'} mb-1`}><b>Phone:</b> {phone}</div>
      )}
      {address && (
        <div className={`${theme === 'light' ? 'text-gray-500' : 'text-gray-500'} mb-1`}><b>Address:</b> {address}</div>
      )}
      {founded > 0 && (
        <div className={`${theme === 'light' ? 'text-gray-500' : 'text-gray-500'} mb-1`}><b>Founded:</b> {founded}</div>
      )}
      {countries.length > 0 && (
        <div className={`${theme === 'light' ? 'text-gray-500' : 'text-gray-500'} mb-1`}>
